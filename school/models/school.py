...
class AcademicYear(models.Model):
    ...
    def _check_academic_year(self):
        ...
        if delta.days > 365 and not calendar.isleap(new_start_date.year):
            raise ValidationError(
                _(
                    """Error! The duration of the academic year
                                      is invalid."""
                )
            )
        if (
        ...
        for old_ac in self.search([("id", "not in", self.ids)]):
            ...
            if (
                ...
                raise ValidationError(
                    _(
                        """Error! You cannot define overlapping
                                          academic years."""
                    )
                )
    ...
    def check_current_year(self):
        ...
        if len(check_year.ids) >= 2:
            raise ValidationError(
                _(
                    """Error! You cannot set two current \
year active!"""
                )
            )
...
class AcademicMonth(models.Model):
    ...
    date_start = fields.Date(
        "Start of Period", required=True, help="Starting of academic month"
    )
    date_stop = fields.Date(
        "End of Period", required=True, help="Ending of academic month"
    )
    ...
    def _check_duration(self):
        ...
        if (
            ...
            raise ValidationError(
                _(
                    """ End of Period date should be greater
    ...
    def _check_year_limit(self):
        ...
        if self.year_id and self.date_start and self.date_stop:
            if (
                ...
                raise ValidationError(
                    _(
                        """Invalid Months ! Some months overlap
...
class SchoolStandard(models.Model):
    ...
    standard_id = fields.Many2one(
        "standard.standard", "Standard", required=True
    )
    division_id = fields.Many2one(
        "standard.division", "Division", required=True
    )
    ...
    syllabus_ids = fields.One2many(
        "subject.syllabus", "standard_id", "Syllabus"
    )
    total_no_subjects = fields.Integer(
        "Total No of Subject", compute="_compute_subject"
    )
    name = fields.Char("Name")
    capacity = fields.Integer("Total Seats")
    total_students = fields.Integer(
        "Total Students", compute="_compute_total_student", store=True
    )
    remaining_seats = fields.Integer(
        "Available Seats", compute="_compute_remain_seats", store=True
    )
    ...
    def check_standard_unique(self):
        ...
        if standard_search:
            raise ValidationError(
                _("""Division and class should be unique!""")
            )
    ...
    def unlink(self):
        for rec in self:
            if rec.student_ids or rec.subject_ids or rec.syllabus_ids:
                raise ValidationError(
                    _(
                        """You cannot delete this standard
    ...
    def check_seats(self):
        ...
        if self.capacity <= 0:
            raise ValidationError(
                _(
                    """Total seats should be greater than
                0!"""
                )
            )
...
class SchoolSchool(models.Model):
    ...
    com_name = fields.Char(
        "School Name", related="company_id.name", store=True
    )
    code = fields.Char("Code", required=True)
...
class SubjectElective(models.Model):
    ...
    subject_ids = fields.One2many(
        "subject.subject", "elective_id", "Elective Subjects"
    )
...
class DocumentType(models.Model):
    ...
    seq_no = fields.Char(
        "Sequence", readonly=True, default=lambda self: _("New")
    )
...
class StudentPreviousSchool(models.Model):
    ...
    def check_date(self):
        ...
        if self.admission_date >= new_dt or self.exit_date >= new_dt:
            raise ValidationError(
                _(
                    """Your admission date and exit date
        ...
        if self.admission_date > self.exit_date:
            raise ValidationError(
                _(
                    """ Admission date should be less than
...
class AcademicSubject(models.Model):
    ...
    add_sub_id = fields.Many2one(
        "student.previous.school", "Add Subjects", invisible=True
    )
    name = fields.Char("Name", required=True)
    maximum_marks = fields.Integer("Maximum marks")
    minimum_marks = fields.Integer("Minimum marks")
...
class StudentFamilyContact(models.Model):
    ...
    relation = fields.Many2one(
        "student.relation.master", "Relation", required=True
    )
    phone = fields.Char("Phone", required=True)
...
class StudentNews(models.Model):
    ...
    def checknews_dates(self):
        ...
        if self.date < new_date:
            raise ValidationError(
                _(
                    """Configure expiry date greater than \
current date!"""
                )
            )
...
specified!"""
        ...
        for news in self:
            ...
            # Check email is defined in user created from employee
                ...
                if not email_list:
                    raise except_orm(
                        _("Email Configuration!"), _("Email not defined!")
                    )
            news_date = news.date
            ...
            obj_mail_server.send_email(
                message=message, mail_server_id=mail_server_ids[0].id
            )
        return True
...
class StudentReminder(models.Model):
    ...
    stu_id = fields.Many2one(
        "student.student", "Student Name", required=True, default=check_user
    )
...
class Report(models.Model):
    ...
    def render_template(self, template, values=None):
        student_id = self.env["student.student"].browse(
            self._context.get("student_id", False)
        )
        if student_id and student_id.state == "draft":
            raise ValidationError(
                _(
                    """You cannot print report for
