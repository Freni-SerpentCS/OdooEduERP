        ...
        if self.date_of_birth:
            ...
            if age_calc < self.school_id.required_age:
                raise ValidationError(
                    _(
                        """Age of student should be greater \
than %s years!"""
...
defined!Please contact to Administator!"""
    ...
    student_name = fields.Char(
        "Student Name", related="user_id.name", store=True, readonly=True
    )
    pid = fields.Char(
        "Student ID",
        required=True,
    ...
    middle = fields.Char(
        "Middle Name", required=True, states={"done": [("readonly", True)]}
    )
    last = fields.Char(
        "Surname", required=True, states={"done": [("readonly", True)]}
    )
    ...
    date_of_birth = fields.Date(
        "BirthDate", required=True, states={"done": [("readonly", True)]}
    )
    mother_tongue = fields.Many2one("mother.toungue", "Mother Tongue")
    age = fields.Integer(
        compute="_compute_student_age", string="Age", readonly=True
    )
    ...
    school_id = fields.Many2one(
        "school.school", "School", states={"done": [("readonly", True)]}
    )
    ...
    teachr_user_grp = fields.Boolean(
        "Teacher Group", compute="_compute_teacher_user"
    )
    active = fields.Boolean(default=True)
        ...
        for rec in self:
            ...
            if not school_standard_obj.search(domain):
                raise except_orm(
                    _("Warning"),
                    _(
                        """The standard is not defined in
                                     school"""
                    ),
                )
