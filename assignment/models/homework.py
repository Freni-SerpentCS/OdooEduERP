    ...
    def unlink(self):
        for rec in self:
            if rec.state != "draft":
                raise ValidationError(
                    _(
                        """You can delete record in unconfirm
                state only!"""
                    )
                )
...
class SchoolStudentAssignment(models.Model):
    ...
    def check_date(self):
        if self.due_date < self.assign_date:
            raise ValidationError(
                _(
                    "Due date of homework should be greater \
                                   than Assign date!"
                )
            )
    ...
    teacher_assignment_id = fields.Many2one(
        "school.teacher.assignment", string="Teachers"
    )
